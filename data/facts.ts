...
const factsDataSource: Record<string, string[]> = {
  "Finance & Investing": [
...
const generateFacts = (): Fact[] => {
  const facts: Fact[] = [];
  const factsPerCategory = TOTAL_FACTS / factCategories.length; // Should be 50
  ...
  for (let i = 1; i <= TOTAL_FACTS; i++) {
    ...
    const factText = factsDataSource[category]?.[factInCategoryIndex % factsPerCategory] 
      || `Default fact for ${category} if specific one is missing. Index: ${factInCategoryIndex}`;
...
const allFacts: Fact[] = generateFacts();

export const getFactById = (id: number): Fact | undefined => {
  if (id < 1 || id > TOTAL_FACTS) {
    return undefined;
  }
